...
model_file_new = os.path.join(project_root, 'data/real_data_new/AGRESTE.mps')
...
# col_sums_of_squares = A.power(2).sum(axis=0)
# column_norms = np.sqrt(np.asarray(col_sums_of_squares).ravel())
# print(column_norms)
# # How many coefficients are zero in c
# c = np.array(c)
# zero_c = np.where(c == 0)[0]
# print(len(zero_c))
