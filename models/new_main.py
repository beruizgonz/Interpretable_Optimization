...
def sensitivity_analysis(model, presolve,  dual = False, min_threshold = 0.015, max_threshold = 0.3, step = 0.2):
    ...
    while threshold <= max_threshold:
        ...
        if len(rows_zero) > 0:
            print('changed')
        #     A_changed1 = np.delete(A_changed1, rows_zero, axis=0)
        #     b = np.delete(b, rows_zero)
        #     A_changed = csr_matrix(A_changed1)
...
def sensitivity_analysis_file(file,save_path):
    ...
    lb, ub = calculate_bounds2(original_primal)
    ...
    results_problem_dual = sensitivity_analysis(model_to_use_dual, presolve_dual, dual = True)
...
if __name__ == '__main__':
    ...
    model_names_2 = ['DANWOLFE.mps', 'DINAM.mps', 'EGYPT.mps', 'INDUS89.mps','FERT.mps', 'ISWNM.mps', 'MSM.mps', 'PHOSDIS.mps', 'PRODSP.mps', 'SARF.mps', 'TABORA.mps', 'TURKPOW.mps']


    all_results = nested_dict()
    sparsification_results = os.path.join(project_root, 'results/epsilon_sparsification_new')
    results_folder = os.path.join(project_root, 'results/epsilon_sparsification')
    zeroepsilon_row_results = os.path.join(project_root, 'results/zeroepsilon_rows_abs')
    ...
    for model_name in models_to_process:
        ...
        results = sensitivity_analysis_file(model_path,save_path = zeroepsilon_row_results)
